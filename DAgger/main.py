...
# ACTION_UP = [[0, 5], [2, 4], [2, 5], [3, 4]]
# ACTION_DOWN = [[0, 0], [1, 0], [1, 1], [2, 2], [4, 0], [4, 1], [4, 2], [5, 3], [5, 4]]
# ACTION_LEFT = [[2, 0], [5, 0]]
# ACTION_RIGHT = [[0, 1], [0, 4], [1, 2], [1, 4], [2, 3], [3, 3], [4, 3]]
...
def run_maze():
    ...
    net_s = tf.placeholder(tf.float32, [None, n_features], name='s')  # observations
    net_a = tf.placeholder(tf.int32, [None, n_actions], name='a')  # expert actions

    net_l1 = tf.contrib.layers.fully_connected(net_s, 128, activation_fn=tf.nn.relu)
    ...
    with tf.Session() as sess:
        ...
        save_path = saver.save(sess, './logs/model/model_init.ckpt')
        print("Model saved in path: {}".format(save_path))

    # # Dataset Aggregation and Retraining the policy.
    # restore_path = save_path
    restore_path = './logs/model/model_init.ckpt'
    ...
    for i in range(dagger_itr):
        ...
        with tf.Session() as sess:
            ...
            save_path = saver.save(sess, './logs/model/model_{}.ckpt'.format(i))
            print("Model saved in path: {}".format(save_path))
